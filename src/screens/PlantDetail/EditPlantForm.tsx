...
import { updatePlant } from '@/src/services/firestore';
...
export default function EditPlantForm() {
  ...
  const { plant, isLoading, error: loadError } = usePlantDetail(id);
  ...
  useEffect(() => {
    if (plant) {
      reset({
        ...
        wateringFrequencyDays: String(plant.careRules?.wateringFrequencyDays ?? 7),
        sunlight: plant.careRules?.sunlight ?? '',
  ...
  const onSubmit = async (data: PlantEditFormInput) => {
    ...
    setIsSaving(true);
    try {
      // Zod transforms string inputs to numbers
  ...
  if (loadError || !plant) {
    return (
      <View style={[s.container, s.center]}>
        <Text style={s.errorText}>{loadError ?? 'No se pudo cargar la planta'}</Text>
      </View>
    );
  }
